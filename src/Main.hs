...
handleDecl d = do
          ...
          Interactive -> 
            case d of
              ...
              (SDecl p b bs t) -> do
                ...
                t'' <- elab $ sDeclBody sd'
                (Decl p'' x tt) <- tcDecl (Decl (sDeclPos sd') (fst $ head $ sDeclTy sd') t'')
                te <- eval tt
                addDecl (Decl p'' x te)
          Typecheck -> do
              f <- getLastFile
              ...
              case d of
                (SType p n t) -> do
                  nt <- typeElab p t
                  addTypeDecl (n,nt)
                  --ppterm <- ppDecl sd  -- cambiar por ppTyDecl
                  --printFD4 ppterm
                (SDecl p b bs t) -> do
                  sd' <- elabSDecl p b bs t
                  t'' <- elab $ sDeclBody sd'
      ...
      -- where
      --  handleSDecl :: MonadFD4 m => SDecl STerm STy-> m (Decl TTerm)
      --  typecheckDecl (SDecl p _ x t) = tcDecl (Decl p  x (elab t))
